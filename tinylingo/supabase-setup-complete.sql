...
CREATE TABLE preset_world_admins (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id VARCHAR(255) NOT NULL,  -- 改为VARCHAR以支持字符串ID
  user_email VARCHAR(255) NOT NULL,
  permissions TEXT[] DEFAULT '{"create", "edit", "delete", "publish"}',
  granted_by VARCHAR(255),  -- 改为VARCHAR以支持字符串ID
...
-- 插入管理员用户数据
-- 注意：这里使用字符串ID 'admin-user-1'，与代码中的默认值保持一致
INSERT INTO preset_world_admins (user_id, user_email, permissions, granted_by, granted_at, is_active) VALUES
('admin-user-1', 'admin@tinylingo.com', '{"create", "edit", "delete", "publish", "manage_admins"}', 'system', NOW(), true);

-- 可选：添加更多管理员用户
-- INSERT INTO preset_world_admins (user_id, user_email, permissions, granted_by, granted_at, is_active) VALUES
-- ('admin-user-2', 'admin2@tinylingo.com', '{"create", "edit", "delete", "publish"}', 'admin-user-1', NOW(), true);
...
-- 检查默认分类数据
SELECT 'Categories inserted' as status, COUNT(*) as category_count FROM preset_categories;

-- 检查管理员权限
SELECT 'Admin users inserted' as status, COUNT(*) as admin_count FROM preset_world_admins;